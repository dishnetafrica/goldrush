                    ...
                    <tbody>
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                ...
                                <td><span class="text--dark">{{ Str::words($item->language?->$system_default_lang?->comment ?? "", 8, '...') }}</span></td>
                                <td>
            ...
            <div class="modal-form-data">
                <form class="modal-form" method="POST" action="{{ setRoute('admin.setup.sections.section.item.update',$slug) }}" enctype="multipart/form-data">
                    ...
                    <div class="row mb-10-none mt-3">
                        ...
                        <input type="hidden" name="old_bg_image" value="{{ old('old_bg_image') }}">
                        <div class="col-xl-12 col-lg-12 form-group">
                            @include('admin.components.form.input-file',[
                                'label'             => __("BG Image").'*',
                                'name'              => "bg_image_edit",
...
@push('script')
    ...
    <script>
        ...
        $(".edit-modal-button").click(function(){
            ...
            editModal.find("input[name=old_bg_image]").val(oldData.bg_image);
            editModal.find("input[name=image_edit]").attr("data-preview-name",oldData.image);
            editModal.find("input[name=bg_image_edit]").attr("data-preview-name",oldData.bg_image);

            fileHolderPreviewReInit("#client-feedback-update input[name=image_edit]");
            fileHolderPreviewReInit("#client-feedback-update input[name=bg_image_edit]");
