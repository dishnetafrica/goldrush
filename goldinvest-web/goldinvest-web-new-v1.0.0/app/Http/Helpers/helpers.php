...
function files_path($slug)
{
    $data = [
        ...
        'transaction'        => [
            'path'      => 'frontend/images/transactions',
        ],
    ];
...
function generate_random_string_number($length = 12, $prefix = "")
{
...
function generate_unique_string($table, $column, $length = 10, $prefix = "")
{
    do {
        $generate_rand_string = generate_random_string_number($length, $prefix);
...
function textsLength($string, $length = 60)
{
    return Illuminate\Support\Str::limit($string, $length);
}

function authWalletBalance($balance_type = 'c_balance')
{
    $wallet = UserWallet::where('user_id', auth()->user()->id)->first();
    $balance = $balance_type === 'p_balance' ? $wallet->profit_balance : $wallet->balance;
...
function totalAdminProfits(){
    ...
    $total_profits = agentOnBaseCurrency(Transaction::complete()->get());
